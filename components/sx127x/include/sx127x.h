...
/*
 * Tự kiểm tra độ tin cậy của đường truyền SPI: ghi rồi đọc lại nhiều lần
 * 1 thanh ghi vô hại (không ảnh hưởng hoạt động radio khi ở standby),
 * đếm số lần đọc khớp với giá trị vừa ghi. In log chi tiết ra Serial.
 * Gọi hàm này ngay sau sx127x_init() để chẩn đoán lỗi "gửi timeout"/
 * hoạt động chập chờn nghi do tín hiệu SPI không ổn định.
 * Trả về true nếu toàn bộ các lần test đều khớp (SPI đáng tin cậy).
 */
