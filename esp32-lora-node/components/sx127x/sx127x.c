...
bool sx127x_init(sx127x_t *dev, spi_host_device_t host,
    ...
    spi_device_interface_config_t devcfg = {
        .clock_speed_hz = 1 * 1000 * 1000, // TAM HA XUONG 1MHz DE CHAN DOAN loi TX timeout - nang lai sau khi on dinh
        .mode = 0,
...
bool sx127x_self_test(sx127x_t *dev) {
    ESP_LOGI(TAG, "Bat dau tu kiem tra do tin cay SPI (ghi/doc 20 lan thanh ghi SYNC_WORD)...");
    const uint8_t gia_tri_test[4] = { 0xAB, 0x55, 0xF0, 0x0F };
    int so_lan_dung = 0;
    const int TONG_SO_LAN = 20;

    for (int i = 0; i < TONG_SO_LAN; i++) {
        uint8_t gia_tri_ghi = gia_tri_test[i % 4];
        write_reg(dev, REG_SYNC_WORD, gia_tri_ghi);
        vTaskDelay(pdMS_TO_TICKS(5));
        uint8_t gia_tri_doc = read_reg(dev, REG_SYNC_WORD);

        if (gia_tri_doc == gia_tri_ghi) {
            so_lan_dung++;
        } else {
            ESP_LOGW(TAG, "  Lan %d: ghi 0x%02x nhung doc lai duoc 0x%02x (SAI)", i + 1, gia_tri_ghi, gia_tri_doc);
        }
    }

    ESP_LOGI(TAG, "Ket qua tu kiem tra: %d/%d lan dung.", so_lan_dung, TONG_SO_LAN);
    if (so_lan_dung < TONG_SO_LAN) {
        ESP_LOGE(TAG, "=> SPI KHONG ON DINH! Loi thuong do: day MOSI/CS long/qua dai, "
                      "moi han module RA-02 kem, hoac nhieu tren nguon 3.3V. "
                      "Day la nguyen nhan goc gay loi 'Gui goi LoRa timeout'.");
    } else {
        ESP_LOGI(TAG, "=> SPI on dinh trong bai test nay. Neu van bi TX timeout, "
                      "nghi ngo module SX1278 hong that su (PA khong hoat dong).");
    }

    return so_lan_dung == TONG_SO_LAN;
}
...
bool sx127x_send_packet(sx127x_t *dev, const uint8_t *data, uint8_t len, uint32_t timeout_ms) {
    ...
    while (true) {
        ...
        if ((xTaskGetTickCount() - start) > pdMS_TO_TICKS(timeout_ms)) {
            uint8_t irq_cuoi_cung = read_reg(dev, REG_IRQ_FLAGS);
            uint8_t op_mode_hien_tai = read_reg(dev, REG_OP_MODE);
            ESP_LOGW(TAG, "Gui goi LoRa timeout. REG_IRQ_FLAGS=0x%02x REG_OP_MODE=0x%02x "
                          "(neu IRQ_FLAGS=0x00 lien tuc -> nghi ngo tin hieu SPI khong on dinh "
                          "hoac module khong that su vao che do TX)",
                     irq_cuoi_cung, op_mode_hien_tai);
            set_mode(dev, MODE_STDBY);
            return false;
        }
